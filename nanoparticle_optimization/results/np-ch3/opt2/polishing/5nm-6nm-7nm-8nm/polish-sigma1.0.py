...
for radius in radii:
    opt_path = os.path.join('results', 'np-ch3', 'opt1', 'brute',
    ...
    target_path = os.path.join('utils', 'target_data', 'np_alkane', 'united_atom',
        'truncated', 'U_{}nm_CH3_truncated.txt'.format(int(radius)))
...
epsilon_vals = local_optimization.grid[1].flatten()
n_vals = local_optimization.grid[0].flatten()
m_vals = local_optimization.grid[2].flatten()
...
sigma = np_opt.Parameter(value=(sigma_bead + sigma_ch3) / 2, fixed=True)
epsilon = np_opt.Parameter(value=epsilon_val, upper=1.0, lower=0.1)
