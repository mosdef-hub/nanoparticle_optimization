...
for radius in radii:
    opt_path = os.path.join('results', 'np-mmm', 'opt1', 'brute',
    ...
    target_path = os.path.join('utils', 'target_data', 'np_alkane', 'coarse_grained',
        'truncated', 'U_{}nm_MMM_truncated.txt'.format(int(radius)))
...
m_vals = local_optimization.grid[0].flatten()
...
sigma = np_opt.Parameter(value=(sigma_bead + sigma_mmm) / 2, fixed=True)
