...
int main(int argc, char *argv[]){
    ...
    int np1_size = atoi(argv[1]);
    int np2_size = atoi(argv[2]);
    
    stringstream filename1, filename2;
    filename1 << np1_size << "nm_nano_AA.xyz";
    filename2 << np2_size << "nm_nano_AA.xyz";
	
    gbb nanoparticle1, nanoparticle2;
	load_np(filename1.str().c_str(), nanoparticle1);
    load_np(filename2.str().c_str(), nanoparticle2);
    
    nanoparticle1.shift_com();
    nanoparticle2.shift_com();
    ...
    double eff_size = (np1_size + np2_size) / 2.0;
    
    double maxx = 20.0*eff_size + 40.0 + 10.0*(ceil(eff_size/2.0) - 1.0);
    double minn = 20.0*eff_size;
	double n_points = 100;
    ...
    trajname << np1_size << "nm-" << np2_size << "nm.lammpstrj";
    ...
    while(not_flat)
    {
        ...
        if(count%10 == 0)
        {
            ...
            stringstream Uname;
            Uname << "U_" << np1_size << "nm-" << np2_size << "nm.txt";
            
            stringstream visname;
            visname << "visited_" << np1_size << "nm-" << np2_size << "nm.txt";
