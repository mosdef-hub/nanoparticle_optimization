...
for radius in radii:
    ...
    initial_data = np.loadtxt('U_{:.0f}nm_CH3_init.txt'.format(radius))

    # Load the additional data obtained around the well region
    all_data = np.loadtxt('U_{:.0f}nm_CH3_well.txt'.format(radius))
    well_data = []
    ...
    # Load the long-range target data obtained
    lr_data = np.loadtxt('U_{:.0f}nm_CH3-lr.txt'.format(radius))
    ...
    total = np.vstack((lr_data, revised_initial_data, well_data))
    ...
    np.savetxt('U_{:.0f}nm_CH3-complete.txt'.format(radius),
