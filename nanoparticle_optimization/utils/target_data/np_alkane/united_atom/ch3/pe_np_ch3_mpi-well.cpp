#include "/raid6/homes/summeraz/group-code/common.hpp"
#include "mpi.h"
...
bool calc_pe_compass(gbb np, double *U, int pid, int n_threads)
{
	...
	double sigma_SiCH3 = 3.83;
	double epsilon_SiCH3 = 0.1596;
	
	double sigma_OCH3 = 3.38;
	double epsilon_OCH3 = 0.1247;
	
	coord_t epsilon;
	coord_t sigma;
	
	epsilon.push_back(epsilon_SiCH3);
	epsilon.push_back(epsilon_OCH3);
	
	sigma.push_back(sigma_SiCH3);
	sigma.push_back(sigma_OCH3);
...
int main(int argc, char *argv[]){
    ...
    double maxx = 10.0*np_size + 4.0;
    double minn = 10.0*np_size + 2.0;
	double n_points = 50;
    ...
    trajname << np_size << "nm_CH3.lammpstrj";
    ...
    while(not_flat)
    {
        ...
        if(count%10 == 0)
        {
            ...
            stringstream Uname;
            Uname << "U_" << np_size << "nm_CH3_well.txt";
            
            stringstream visname;
            visname << "visited_" << np_size << "nm_CH3_well.txt";
        ...
        if(count%500 == 0)
        {
            ...
            gbb cross_particle;
            cross_particle.insert_particle(origin, 15.0345, 3, 0);
